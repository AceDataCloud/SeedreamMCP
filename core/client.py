...
class SeedreamClient:
    """Async HTTP client for AceDataCloud Seedream API."""

    def __init__(self, api_token: str | None = None, base_url: str | None = None):
        """Initialize the Seedream API client.
        ...
        logger.info(f"SeedreamClient initialized with base_url: {self.base_url}")
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        if status in (401, 403):
            raise SeedreamAuthError(message)
        raise SeedreamAPIError(message=message, code=code, status_code=status)
    ...
    async def request(
        ...
        Args:
            endpoint: API endpoint path (e.g., "/seedream/images")
        ...
        Raises:
            SeedreamAuthError: If authentication fails
            SeedreamAPIError: If the API request fails
            SeedreamTimeoutError: If the request times out
        """
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    if isinstance(data, list):
                        logger.info(f"   Returned {len(data)} image(s)")
                        for i, item in enumerate(data, 1):
                            if "image_url" in item:
                                logger.info(f"   🖼️ Image {i}: {item.get('image_url', 'N/A')}")
                else:
    ...
    async def query_task(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return await self.request("/seedream/tasks", kwargs)
